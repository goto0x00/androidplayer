...
import dai.android.virtual.utility.Dex;
import dai.android.virtual.utility.PackageParserCompat;
...
public final class LoadedPlugin {
    ...
    public static final String TAG = LoadedPlugin.class.getSimpleName();
    ...
    private static AssetManager createAssetManager(Context context, File apk) {
        try {
            ...
            Reflect.invoke(AssetManager.class, am, "addAssetPath", apk.getAbsolutePath());
            return am;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    private static ResolveInfo chooseBestActivity(Intent intent,
                                                  String s,
                                                  int flags,
    ...
    LoadedPlugin(PluginManager pluginManager, Context context, File apk)
        ...
        this.mPackageInfo.instrumentation =
                instrumentations.values().toArray(new InstrumentationInfo[instrumentations.size()]);
        ...
        this.mPackageInfo.activities =
                activityInfos.values().toArray(new ActivityInfo[activityInfos.size()]);
        ...
        this.mPackageInfo.providers =
                providerInfos.values().toArray(new ProviderInfo[providerInfos.size()]);
        ...
        this.mPackageInfo.receivers =
                receivers.values().toArray(new ActivityInfo[receivers.size()]);
    }
    ...
    private void tryToCopyNativeLib(File apk) {
        ...
        if (metaData != null && metaData.getBoolean("VA_IS_HAVE_LIB")) {
            Plugin.copyNativeLib(apk, mHostContext, mPackageInfo, mNativeLibDir);
        }
    }
    ...
    public Intent getLeanbackLaunchIntent() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            launcher = new Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LEANBACK_LAUNCHER);
        } else {
            launcher = new Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER);
        }
    ...
    public Resources.Theme getTheme() {
        ...
        theme.applyStyle(Plugin.selectDefaultTheme(
                this.mPackage.applicationInfo.theme,
    ...
    public void setTheme(int resid) {
        try {
            Reflect.setField(Resources.class, this.mResources, "mThemeResId", resid);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private class PluginPackageManager extends PackageManager {
        ...
        public int getPackageUid(String s, int i) throws NameNotFoundException {
            Object uid = Reflect.invokeNoException(PackageManager.class, mHostPackageManager, "getPackageUid",
